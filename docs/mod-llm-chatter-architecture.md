...
| `src/LLMChatterGroup.cpp` | ~1350 | Shared group state definitions, shared helpers (`GroupHasRealPlayer`, `GetRandomBotInGroup`, `CountBotsInGroup`, pre-cache helpers), MultiBot-Chatless `MBOT` compatibility handler, named-boss cache, `CleanupGroupSession()` coordinator, thin `LLMChatterGroupPlayerScript` shell wrappers, registration |
...
- shared helpers: `GroupHasRealPlayer`, `GetRandomBotInGroup`,
  ...
  helpers
- MultiBot-Chatless compatibility when
  `LLMChatter.MultiBotCompat.Enable = 1`: answers startup packets
  (`HELLO`, `PING`, `GET~ROSTER`, `GET~STATE(S)`,
  `GET~DETAIL(S)`) and leaves other `MBOT` protocol packets for the
...
| MultiBot-Chatless `MBOT` compatibility | `src/LLMChatterGroup.cpp`; debug skip wording in `src/LLMChatterGroupCombat.cpp` |
